...
model_id = "/lamport/shared/hzheng/workspace/NNs/finetune/quantized-training/examples/gpt2/output"
...
def main(args):
    ...
    seq_len = None #torch.export.Dim("seq_length", min=3, max=args.max_length)
    ...
    # ----------------------------------------------------------------------------------------------
    def calibrate(model):
        validation = load_dataset(dataset_name, dataset_config_name, split="validation")
    ...
    test = load_dataset(dataset_name, dataset_config_name, split="test")
